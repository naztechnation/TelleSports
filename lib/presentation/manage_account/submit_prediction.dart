...
class _SubmitPredictionState extends State<SubmitPrediction> {
  ...
  Widget _buildHomeScoreField(BuildContext context) {
    return Padding(
        padding: EdgeInsets.only(left: 0.h),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Home Score *", style: theme.textTheme.titleSmall),
          SizedBox(height: 3.v),
          CustomTextFormField(
  ...
  Widget _buildAwayScoreField(BuildContext context) {
    return Padding(
        padding: EdgeInsets.only(left: 0.h),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Away Score *", style: theme.textTheme.titleSmall),
          SizedBox(height: 3.v),
          CustomTextFormField(
              ...
              onChanged: (value) {
                if (awayScoreController.text.isNotEmpty) {
                  if (homeScoreController.text.isNotEmpty &&
                      homeTeamController.text.isNotEmpty &&
                      awayTeamController.text.isNotEmpty) {
                    int? homeScore = int.tryParse(homeScoreController.text);
                    int? awayScore = int.tryParse(awayScoreController.text);
                    if (homeScore != null && awayScore != null) {
                      if (homeScore > awayScore) {
                        predictedWinnerController.text =
                            homeTeamController.text;
                      } else if (homeScore < awayScore) {
                        predictedWinnerController.text =
                            awayTeamController.text;
                      } else {
                        predictedWinnerController.text = 'Draw match';
                      }
                    } else {
                      Modals.showToast('Invalid input');
                    }
                  } else {
                    Modals.showToast('fill in all the fields above');
                    awayScoreController.clear();
                  }
                } else {
                  predictedWinnerController.clear();
                }
              },
              contentPadding:
                  EdgeInsets.only(left: 8.h, top: 14.v, bottom: 14.v))
  ...
  Widget _buildOddsField(BuildContext context) {
    return Padding(
        padding: EdgeInsets.only(left: 0.h),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Winner\'s Odd  (Optional)", style: theme.textTheme.titleSmall),
          SizedBox(height: 3.v),
          CustomTextFormField(
        ...
        FilteringTextInputFormatter.allow(
          // RegExp(r'^\d{0,2}(\.\d{0,2})?$'), // Allows up to two digits before and after the decimal point
          RegExp(r'^([0-9]|10)(\.[0-9]{0,2})?$'), // Uncomment this line to limit input to 0-10 range
        ),
      ],
      ...
      if (homeTeamController.text == awayTeamController.text) {
        ...
        if (_formKey.currentState!.validate()) {
          _predictionCubit.postPrediction(
              ...
              homeScore: homeScoreController.text,
              predictedWinner: predictedWinnerController.text,
              awayScore: awayScoreController.text,
              odds: winnerOddController.text,
  ...
  optionWidget2(List<Map<String, String>>? options, String title,
      final controller, bool isHome) {
    return Column(
      children: [
        SizedBox(
          height: 15,
