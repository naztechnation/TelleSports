...
class RequestedUsersPage extends StatelessWidget {
 final List<UserModel> item;
  const RequestedUsersPage({Key? key, required this.item})
      : super(
          key: key,
        );
    ...
    return SafeArea(
      child: Scaffold(
        ...
        // AppNavigator.pushAndStackPage(context, page: IndividualUserInfo(name: groupInfo.groupMembers[index].name, image: groupInfo.groupMembers[index].profilePic, bio: '', username: groupInfo.groupMembers[index].name, isGroupAdmin: index == 0, ));
                ...
                child: RequestDeleteInfo(name: item[index].name, bio: item[index].bio, index: index, image: groupInfo.requestedMembers[index].profilePic, isDelete: false, userId: groupInfo.requestedMembers[index].uid,));
            },
          ),
        ),
      ),
